...
function getUsersFromDatabase(array) {
  ...
  let result = Array.from(array, (user) => ({
    id: user.id,
...
function filterByCondition(arr, condition) {
  ...
  let filterArray = arr.filter(condition);
  return filterArray;
}
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  let check = Array.isArray(arr1, arr2);
  if (check === true) {
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  let newArray = arr.copyWithin(target, start, end);
...
function customUnshift(arr, ...elements) {
  ...
  if (Array.isArray(arr)) {
    ...
    for (let i = arr.length - 1; i >= 0; i++) {
      ...
      return {initialLength: initialLength, newLength: newLength, arr: arr};
    }
  }
}
...
function customAt(arr, index) {
  ...
  if (!Array.isArray(arr) || (index > arr.length && index < arr.length)) {
    ...
    let typeOfElement = arr.at(index);
    if (typeof typeOfElement === "string") {
      console.log("Елемент є рядком")
    } if (typeof typeOfElement === "number"){
      console.log("Елемент є числом")
    } if (typeof typeOfElement === "object"){
...
function customIncludes(arr, element) {
  ...
  if(Array.isArray(arr) === true) {
    let checkArray = arr.includes(element);
    let checkElement = arr.filter((item) => item === element).length;
